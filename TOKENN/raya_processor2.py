...
class TokenProcessor:
    ...
    def get_triage_info(self, report):
        ...
        # 1. Ontology Scoring System for Department
        ontologies = {
            "CARDIOLOGY": ["heart", "cardiac", "palpitation", "angina", "chest pain"],
            "ORTHOPEDICS": ["fracture", "bone", "joint", "sprain", "ligament", "ortho", "broken", "twisted", "knee", "spine", "shoulder", "arthritis", "muscle", "back pain", "neck pain", "sciatica", "disc", "cartilage", "tendon", "spasm", "cramp", "swelling", "hip", "ankle", "wrist", "arm", "leg", "pelvis", "calf"],
            "NEUROLOGY": ["headache", "migraine", "brain", "seizure", "dizzy", "vertigo", "faint", "numbness", "tingling", "paralysis", "concussion", "head injury", "tremor", "nerve", "stroke", "memory loss"],
            "GASTROENTEROLOGY": ["stomach", "abdomen", "gastric", "vomit", "puke", "diarrhea", "liver", "constipation", "nausea", "heartburn", "acid reflux", "ulcer", "food poisoning", "loose motion", "bowel", "intestine", "digestion"],
            "DENTAL DEPT": ["tooth", "dental", "gum", "dentist", "cavity", "toothache", "root canal", "jaw pain", "bleeding gums"],
            "OPHTHALMOLOGY": ["eye", "vision", "blur", "blind", "cataract", "conjunctivitis", "squint", "dry eye", "red eye"],
            "ENT DEPT": ["ear", "nose", "throat", "hearing", "deaf", "sinus", "tonsil", "sore throat", "nosebleed", "swallow", "cough", "phlegm", "stuffy"],
            "DERMATOLOGY": ["skin", "rash", "itch", "acne", "pimple", "allergy", "hives", "eczema", "burn", "hair loss", "blister", "dermatitis", "lesion", "psoriasis", "mole"],
            "GYNECOLOGY": ["pregnancy", "pregnant", "period", "menstruation", "vaginal", "uterus", "cramp", "gynecology", "maternity", "miscarriage", "ovary", "menopause"]
        }

        # Calculate scores for each department
        scores = {}
        for d, keywords in ontologies.items():
            count = sum(1 for kw in keywords if kw in rep)
            if count > 0:
                scores[d] = count
        ...
        # 2. Dynamic Priority / Severity Calculation
        emergency_kw = ["stroke", "unconscious", "passed out", "heart attack", "gasping", "can't breathe", "severe chest pain", "choking", "bleeding heavily", "suicide", "poison"]
        urgent_kw = ["fracture", "broken", "concussion", "head injury", "seizure", "migraine", "faint", "vertigo", "acute", "severe", "paralysis", "miscarriage", "burn", "tear", "bleeding"]

        color, level = (0, 128, 0), "NORMAL (GREEN)"

        if any(k in rep for k in emergency_kw):
            # If critical, override department to Emergency unless it's specialized neuro/cardio block
            if dept not in ["NEUROLOGY", "CARDIOLOGY"]:
                dept = "EMERGENCY DEPT"
            color, level = (255, 0, 0), "EMERGENCY (RED)"
        elif any(k in rep for k in urgent_kw):
            color, level = (255, 165, 0), "URGENT (YELLOW)"
            
        # Cardiology is generally at least URGENT if matched
        if dept == "CARDIOLOGY" and level == "NORMAL (GREEN)":
            color, level = (255, 165, 0), "URGENT (YELLOW)"
