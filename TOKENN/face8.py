...
if os.path.exists(os.path.join(PARENT_DIR, "faces5.db")):
    FACES_DIR = os.path.join(PARENT_DIR, "user_faces")
    DB_PATH = os.path.join(PARENT_DIR, "faces5.db")
else:
    FACES_DIR = os.path.join(BASE_DIR, "user_faces")
    DB_PATH = os.path.join(BASE_DIR, "faces5.db")
...
def speak(text, lang_code=None):
    ...
    if not success:
        print("Speech error: Failed to connect to gTTS after 3 attempts.")
        return
...
def listen(timeout_sec=10, phrase_time=5, lang_code=None, boolean_mode=False):
    if lang_code is None:
        lang_code = LANG
    ...
    try:
        with sr.Microphone(device_index=WORKING_MIC_INDEX) as source:
            ...
            play_beep()
            # Use record() instead of listen() to match raya_interaction's robust strategy
            rec_duration = 3 if boolean_mode else phrase_time
            print(f"[Listening for {rec_duration} seconds...]")
            audio = recognizer.record(source, duration=rec_duration)
            
            if boolean_mode:
                pad_dur = 0.3
                silent_bytes = b'\x00' * int(audio.sample_rate * audio.sample_width * pad_dur)
                tricked_audio_data = audio.frame_data + silent_bytes + audio.frame_data + silent_bytes + audio.frame_data
                processed_audio = sr.AudioData(tricked_audio_data, audio.sample_rate, audio.sample_width)
            else:
                processed_audio = audio
            
            raw_res = recognizer.recognize_google(processed_audio, language=lang_code, show_all=True)
            print(f"[Raw Output]: {raw_res}")
            
            if isinstance(raw_res, dict) and 'alternative' in raw_res and len(raw_res['alternative']) > 0:
                text = raw_res['alternative'][0]['transcript']
            elif isinstance(raw_res, list) and len(raw_res) > 0:
                text = raw_res[0].get('transcript', '')
            else:
                text = ""
...
yes_words = ['yes', 'yep', 'yeah', 'haan', 'ha', 'han', 'ji', 'jee', 'ok', 'right', 'correct', 'y', 'theek', 'हां', 'जी', 'ठीक']
no_words = ['no', 'nope', 'nah', 'nahi', 'nahin', 'na', 'wrong', 'incorrect', 'n', 'galat', 'नहीं', 'ना']
...
def interaction_flow(target_encoding):
    ...
    global recognized_name, recognized_photo_path

    try:
        display_text1 = "Language Selection"
        display_text2 = "Speak 'English' or 'Hindi'"
        
        speak("नमस्ते, आप हिंदी में बात करना चाहेंगे या इंग्लिश में?", lang_code='hi')
        lang_response = listen(lang_code='hi-IN')
        
        if 'english' in lang_response.lower() or 'इंग्लिश' in lang_response:
            LANG = 'en'
            speak("I have set the language to English.", lang_code='en')
        else:
            LANG = 'hi'
            speak("मैंने हिंदी सेट कर दी है।", lang_code='hi')
        ...
        while True:
            ...
            conf_resp = listen(lang_code='en-US' if LANG=='en' else 'hi-IN', boolean_mode=True)
        ...
        while True:
            ...
            photo_resp = listen(lang_code='en-US' if LANG=='en' else 'hi-IN', boolean_mode=True)
            ...
            if is_affirmative(photo_resp):
                ...
                time.sleep(0.5)
                recognized_name = clean_name
                recognized_photo_path = filename
...
def greet_known_user(name):
    global program_running, recognized_name, recognized_photo_path
    speak(f"Namaste {name}", lang_code=LANG)
    time.sleep(0.5)
    recognized_name = name
    cursor.execute("SELECT photo_path FROM users WHERE name=?", (name,))
    row = cursor.fetchone()
    if row and row[0]:
        p = row[0]
        if not os.path.isabs(p):
            p = os.path.join(os.path.dirname(DB_PATH), p)
        recognized_photo_path = p
    program_running = False
...
def run_face_recognition():
    ...
    global recognized_name, recognized_photo_path, is_existing_user
    program_running = True
    session_active = False
    recognized_name = None
    recognized_photo_path = None
    ...
    while program_running:
        ...
        if session_active:
            ...
            if display_text1 in ["Get Ready for Photo", "Taking photo in..."]:
                cx, cy = w//2, h//2
                sz = 300
                y1 = max(0, cy-sz)
                y2 = min(h, cy+sz)
                ...
                step_y = (y2 - y1) // 3
                for i in range(1, 3):
                    cv2.line(frame, (x1 + i * step_x, y1), (x1 + i * step_x, y2), (0, 255, 255), 1)
                    cv2.line(frame, (x1, y1 + i * step_y), (x2, y1 + i * step_y), (0, 255, 255), 1)
                    
                # Instruct user to fit their face inside the grid
                cv2.putText(frame, "Align face in box", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        ...
        cv2.imshow("RAYA - Vision 7", frame)
    ...
    return recognized_name, recognized_photo_path, is_existing_user, LANG

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RAYA Vision 7 System")
    ...
    if args.register:
        if not args.name:
            ...
            print('Example usage: python face7.py --register "photo.jpg" --name "Aditya Singh"')
            sys.exit(1)
        else:
